...
package net.mcreator.laboratory.particle;
...
public class DumbFaceParticle {
	...
	public static void registerParticleType(RegistryEvent.Register<ParticleType<?>> event) {
		event.getRegistry().register(particle.setRegistryName("dumb_face"));
	}
	...
	private static class CustomParticle extends SpriteTexturedParticle {
		...
		protected CustomParticle(ClientWorld world, double x, double y, double z, double vx, double vy, double vz, IAnimatedSprite spriteSet) {
			...
			this.motionY = vy * 1;
			this.motionZ = vz * 1;
			this.angularVelocity = (float) 0.628;
		...
		public void tick() {
			super.tick();
			if (this.InitialSetting == 1) {
				this.particleScale += 0.06f;
				if (this.particleScale >= 0.3f)
					this.InitialSetting = 0;
			} else {
